...
当前截图工具条全部启用：矩形、箭头、画笔、文字、马赛克标注工具 + 撤销/重做 + 取消/贴图/保存/复制。标注通过 AnnotationLayer 组件（Canvas）实现，支持 Ctrl+Z/Y 撤销重做、Space 隐藏工具条。保存支持 Ctrl+S 弹出文件对话框、Ctrl+Shift+S 快速保存到 Pictures/Screenshots 目录。截图模式下移动鼠标会自动检测光标下方的窗口区域并高亮（绿色边框），单击即可吸附到检测到的窗口区域，拖拽则切换为手动选区。选区阶段显示像素级放大镜（10x），含十字准星、坐标和颜色拾取（Hex）。审查阶段支持方向键移动选区、Ctrl+方向键扩大、Shift+方向键缩小。成功截图自动写入 screenshot_history 表（区域坐标+缩放比+文件路径），截图模式下支持 R 重复上次区域、逗号/句号切换历史记录。capture_screen 支持按显示器物理坐标匹配目标屏幕，snip 窗口定位到光标所在显示器，窗口检测自动补偿显示器偏移。
